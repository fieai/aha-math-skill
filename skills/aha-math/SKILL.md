...
2. **分镜先行，必须用户确认（强制）。** 第一步永远只输出分镜脚本（storyboard），交给用户确认或修改；**在得到用户明确确认前，禁止进入视频/网页生成与渲染。** 仅当用户明确说“跳过确认/直接做”时才例外。
...
校验通过后，**把分镜脚本交付给用户，并停在这里等待确认**：

- 用易读形式呈现分镜（镜头清单 + 每镜的画面/旁白/动作/抽象层级），不要只丢一个 JSON 文件路径。
- 明确请用户**确认或提出修改**；用户改了就回到 B 重写并重新校验，再次请确认。
- **在用户明确确认前，禁止进入 C/D、禁止安装环境、禁止渲染任何视频或网页。**
- 唯一例外：用户明确说“跳过确认/直接做”。
- 表单为 `storyboard-only` 时，交付分镜即结束。
