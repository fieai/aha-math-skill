...
2. **先确认讲解方案，再渲染（强制）。** 第一步永远先把“打算怎么讲”交给用户确认：**默认给『讲解步骤』人话视图**（答案 + 编号步骤 + 口诀，不出现镜头/画面/动画/抽象层级等术语），面向普通家长。强类型分镜（storyboard）作为**内部产物**生成并校验，用户要看才展开。未确认前禁止渲染；仅当用户明确说“跳过确认/直接做”才例外。
...
校验通过后，**把“打算怎么讲”交给用户确认，并停在这里等待**。默认面向普通家长，用『讲解步骤』人话视图，**不要展示镜头/画面/动画/抽象层级等术语，也不要只丢一个 JSON 路径**：

- **答案**：先给最终答案，让家长确认对错。
- **关键点**：一句话点出最容易误解、这次要讲通的地方。
- **讲解步骤**：3–5 条编号步骤，用孩子/家长能懂的话写“会怎么一步步讲”（对应分镜推导主线，但不出现术语；具体动作先行）。
- **口诀**：一句可被孩子复述的口头规则。
